...
public partial class SettingsPageView : UserControl
{
    ...
    private async void OnExportGrades(object? sender, RoutedEventArgs e)
        => await Guarded.RunAsync("export your grades", async () =>
        {
            if (Vm is { } vm)
                await SaveText("export grades", $"tomoru-grades-{DateTime.Now:yyyy-MM-dd}.csv",
    ...
    private async void OnExportFocus(object? sender, RoutedEventArgs e)
        ...
        {
            if (Vm is { } vm)
                await SaveText("export focus history", $"tomoru-focus-{DateTime.Now:yyyy-MM-dd}.csv",
    ...
    private async void OnExportTimetable(object? sender, RoutedEventArgs e)
        ...
        {
            if (Vm is { } vm)
                await SaveText("export timetable", $"tomoru-timetable-{DateTime.Now:yyyy-MM-dd}.ics",
                    "calendar", "*.ics", vm.BuildTimetableIcs());
        });

    private async void OnBackup(object? sender, RoutedEventArgs e)
        => await Guarded.RunAsync("write the backup", async () =>
        {
            if (Vm is { } vm)
                await SaveText("backup data", $"tomoru-backup-{DateTime.Now:yyyy-MM-dd}.json",
                    "json", "*.json", vm.BuildBackupJson());
        });

    private async void OnRestoreBackup(object? sender, RoutedEventArgs e)
        => await Guarded.RunAsync("restore that backup", async () =>
        {
            if (Vm is not { } vm)
                return;
            ...
            {
                vm.RestoreStatus = "that file is too large to be a tomoru backup";
                return;
            }
    ...
    private async void OnChooseMusicFolder(object? sender, RoutedEventArgs e)
        => await Guarded.RunAsync("open that music folder", async () =>
        {
            if (Vm is not { } vm)
                return;
