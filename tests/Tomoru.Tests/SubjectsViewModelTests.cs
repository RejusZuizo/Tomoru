...
public class SubjectsViewModelTests
{
    ...
    private static (SubjectsViewModel Vm, AppState State, ConfirmDeleteViewModel Confirm) Vm(params Subject[] subjects)
    {
        var state = new AppState();
        state.Subjects.AddRange(subjects);
        var confirm = new ConfirmDeleteViewModel();
        return (new SubjectsViewModel(state, () => { }, _ => { }, confirm), state, confirm);
    }
    ...
    {
        var (vm, state, confirm) = Vm(Subject("Algorithms", 80, 90));
    ...
    {
        var (vm, _, confirm) = Vm(Subject("Algorithms", 80, 90, 75));

        vm.RemoveCommand.Execute(vm.Items[0]);

        Assert.Equal("Algorithms", confirm.Name);
        Assert.Contains("3 assessments", confirm.Detail);
    ...
    {
        var (vm, _, confirm) = Vm(Subject("Just added"));

        vm.RemoveCommand.Execute(vm.Items[0]);

        Assert.DoesNotContain("assessment", confirm.Detail);
        Assert.Contains("can't be undone", confirm.Detail);
    }
    ...
    {
        var (vm, _, confirm) = Vm(Subject("Algorithms", 80));

        vm.RemoveCommand.Execute(vm.Items[0]);

        Assert.Contains("1 assessment ", confirm.Detail);
    }

    [Fact]
    public void Confirming_removes_it()
    {
        var (vm, state, confirm) = Vm(Subject("Algorithms", 80), Subject("Statistics", 60));
    ...
    {
        var (vm, state, confirm) = Vm(Subject("Algorithms", 80));
    ...
    public void The_grade_scale_moved_out_but_still_answers()
    {
        ...
        var (vm, _, _) = Vm(Subject("Algorithms", 80));
