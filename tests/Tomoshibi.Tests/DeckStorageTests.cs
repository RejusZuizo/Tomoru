...
/// <summary>Decks moved out of tomoshibi.json into their own file, so the
...
public class DeckStorageTests : IDisposable
{
    ...
    {
        _dir = Path.Combine(Path.GetTempPath(), "tomoshibi-decks-" + Guid.NewGuid().ToString("N"));
    ...
    private string StatePath => Path.Combine(_dir, "tomoshibi.json");
